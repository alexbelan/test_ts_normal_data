...
type PostsResult = Pick<Posts, 'byId'>
type PostsIds = Pick<Posts, 'allIds'>

const normalizeData = <T extends Post>(unnormalizedData: T[]): Posts => {
    const arrIds: PostsIds = {
        allIds: []
    }
    const postsResult: PostsResult = {
        byId: {}
    }
    unnormalizedData.forEach((item: Post) => {
        postsResult.byId[item.id] = item
        arrIds.allIds.push(item.id)
    })
    return {
        ...postsResult,
        ...arrIds
